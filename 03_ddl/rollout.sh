  ...
  if [ "${RUN_MODEL}" != "cloud" ]; then
    for i in $(find "${PWD}" -maxdepth 1 -type f -name "*.ext_tpch.*.sql" -printf "%f\n" | sort -n); do
     ...
     if [ "${RUN_MODEL}" == "remote" ]; then
      ...
      log_time "psql ${PSQL_OPTIONS} -v ON_ERROR_STOP=1 -q -a -P pager=off -f ${PWD}/${i} -v LOCATION=\"${LOCATION}\""
      psql ${PSQL_OPTIONS} -v ON_ERROR_STOP=1 -q -a -P pager=off -f ${PWD}/${i} -v LOCATION="${LOCATION}" -v DB_EXT_SCHEMA_NAME="${DB_EXT_SCHEMA_NAME}"
      print_log
    done
  fi
fi
